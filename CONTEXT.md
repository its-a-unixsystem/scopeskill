...
- `sv-cli offene-posten rebook` moves an **Offene Posten** entry between **Personenkonten** on an explicitly selected Debitor or Kreditor side; `sv-cli offene-posten set-reminder-level` updates Mahnstufen by Belegnummer and exits non-zero when Scopevisio reports any per-item failure.
- The **`sv-cli`** stitches data on `show`-style commands when the second piece is reliably co-requested, but never on list-style commands (N+1 risk) and never derives business answers except the verified **Buchung** lifecycle required to distinguish an unchanged cancelled original from an active **Buchung** (see ADR-0006).
