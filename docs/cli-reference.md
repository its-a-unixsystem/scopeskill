...
- `sv-cli offene-posten clear --seite=debitor|kreditor --data @clearing.json [--dry-run] [--allow-partial] [--yes]`
  Safely allocate one reviewed payment to one or more explicit open items on
  the selected Debitor or Kreditor side. The normal CLI-owned JSON shape is
  ...
  cross-Personenkonto allocations,
  ...
  rechecks the live balances, sends at most one write to the selected side's
  clearing endpoint, and reads every balance back. Stdout
