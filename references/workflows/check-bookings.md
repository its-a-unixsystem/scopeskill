...
* **Flag Discrepancies:** Personal account balance $\neq$ sum of open items. Resolve `<bookingNumber>` from the Buchung's `documentNumber` or an Offene-Posten `postingNumber` that identifies it; otherwise use the Journal or Personenjournal rather than inventing an identifier. Run `sv-cli buchung show <bookingNumber>` and treat it as active only when `lifecycle.state` is `active`. When cancelled, read `lifecycle.cancellationDocumentNumber`, `lifecycle.cancellation`, and optional `lifecycle.replacementDocumentNumber`; stop on `conflict`.
